  ...
  <HTML>
   ...
   <BODY style="margin-top:250px;">
    ...
    <TABLE border="1">
     ...
     <xsl:for-each  select="users/Usuario">
       <xsl:sort order="ascending" select="ranking/pos" data-type="number"/>
      <TR>
      <TD><xsl:value-of select="ranking/pos"/></TD>
       <TD><xsl:value-of select="nick"/></TD>
       <TD><xsl:value-of select="name"/> <xsl:value-of select="surname"/></TD>
       <TD><xsl:value-of select="ranking/country"/></TD>
       <TD><xsl:value-of select="ranking/points"/></TD>
       <BR></BR>
      </TR>
     </xsl:for-each>
    </TABLE>
